...
class AlertEngine:
    ...
    def __init__(self, config: dict, hp_logger: HoneypotLogger):
        ...
        # DDoS / bağlantı seli
        self.ddos_connections = thresholds.get("ddos_connections", 30)
        self.ddos_window = thresholds.get("ddos_window_sec", 10)
        self.global_ddos_connections = thresholds.get(
            "global_ddos_connections", 200
        )
        self.global_ddos_window = thresholds.get(
        ...
        self._global_connections = []              # tüm bağlantılar
    ...
    def track_connection(self, src_ip: str, service: str, port: int = 0):
        """
        Yeni bir bağlantıyı izle. Tek IP'den veya toplam üzerinden
        eşik aşılırsa DDoS / connection flood alarmı üret.
        """
        now = time.time()
        with self._lock:
            self._connections[src_ip].append(now)
            self._global_connections.append(now)

            # Pencere dışını temizle
            self._connections[src_ip] = [
                t for t in self._connections[src_ip]
                if now - t <= self.ddos_window
            ]
            self._global_connections = [
                t for t in self._global_connections
                if now - t <= self.global_ddos_window
            ]

            ip_count = len(self._connections[src_ip])
            global_count = len(self._global_connections)

            if ip_count >= self.ddos_connections:
                key = f"flood_{src_ip}_{service}_{int(now // self.ddos_window)}"
                if key not in self._seen_alerts:
                    ...
                    self.process_alert(
                        ...
                        src_ip=src_ip,
                        description=(
                            f"Bağlantı seli (flood) tespit edildi "
                            f"[{service.upper()}]: "
                            f"{ip_count} bağlantı / {self.ddos_window}s"
                        ),
                        evidence={
                            "service": service,
                            "connections_in_window": ip_count,
                            "window_seconds": self.ddos_window,
                            "threshold": self.ddos_connections,
                            "port": port,
                        },
                    )

            if global_count >= self.global_ddos_connections:
                key = f"ddos_global_{int(now // self.global_ddos_window)}"
                if key not in self._seen_alerts:
                    self._seen_alerts.add(key)
                    unique_ips = len({
                        ip for ip, ts in self._connections.items() if ts
                    })
                    self.process_alert(
                        alert_type="ddos",
                        severity="critical",
                        src_ip=src_ip,
                        description=(
                            f"DDoS tespit edildi: {global_count} bağlantı "
                            f"/ {self.global_ddos_window}s — "
                            f"{unique_ips} farklı IP"
                        ),
                        evidence={
                            "total_connections": global_count,
                            "unique_ips": unique_ips,
                            "window_seconds": self.global_ddos_window,
                            "threshold": self.global_ddos_connections,
                        },
                    )
